...
4. 选择完成后焦点自动回到当前会话输入框，无需鼠标重新点击；
5. 用户继续输入任务，并再次手动发送；
6. 选择阶段不调用 `remote.commands.execute`。
...
键盘行为由 DSH 公共 `popupSelect` 外壳负责：弹窗打开后焦点应在筛选框，`↑`/`↓` 只移动候选高亮，回车只确认候选。插件会在候选确认并回填草稿后兼容恢复输入框焦点，因此用户可以直接继续输入并手动发送。若 `↓` 正常而 `↑` 触发消息浏览，仍属于 DSH 公共弹窗的事件传播问题，应在 `PopupSelectView` 的 keydown 处理处修复并在公共组件测试中回归。
